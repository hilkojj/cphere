...
struct Placed
{
    ...
    Placed(Blueprint &bp, int x, int y, int rotation, Island *on, mat4 transform=mat4(0))
...
class BuildingSystem : public LevelSystem
{
  public:
    ...
    {
        lvl->registry.on_construct<Placed>().connect<&BuildingSystem::onPlaced>(this);
        lvl->registry.on_destroy<Placed>().connect<&BuildingSystem::onDestroyed>(this);
    ...
    void update(double deltaTime, Level *lvl) override
    {
        ...
        lvl->registry.view<Blueprint>().each([&](auto e, Blueprint &bp) {
            ...
            {
                lvl->registry.reset<Placing>();
                lvl->registry.assign<Placing>(e);
                std::cout << "Start placing of " << bp->name << "\n";
            }
        });

        lvl->registry.view<Placing, Blueprint>().each([&](auto entity, Placing &placing, Blueprint &bp) {
            ...
            if (isl)
            {
                ...
                if (isl->tileUnderCursor(tile, lvl->cam))
                {
                    ...
                    placing.blocked = !canPlace(bp, placing.x, placing.y, placing.rotation, placing.on);
                }
            }
    ...
    static bool canPlace(Blueprint &bp, int x, int y, int rotation, Island *isl)
    {
        Placed p(bp, x, y, rotation, isl);
        for (auto &t : p.tiles)
        {
    ...
    static void place(Blueprint &bp, entt::registry &reg, int x, int y, int rotation, Island *on)
    {
        ...
        reg.assign<Placed>(newEntity, bp, x, y, rotation, on);
  ...
  private:
    void onPlaced(entt::registry &reg, entt::entity entity, Placed &placed)
    {
    ...
    {
        auto &placed = reg.get<Placed>(entity);
        auto &blueprint = reg.get<Blueprint>(entity);
        std::cout << blueprint->name << " DESTROYED at " << to_string(placed.tiles[0]) << " on Island " << placed.on << "\n";
